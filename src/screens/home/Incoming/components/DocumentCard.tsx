...
const DocumentCard = React.memo(
    ({
        ...
        const status =
            STATUS_COLOR[item.statusLabel] || STATUS_COLOR['Bản nháp'];
        const cardAction = getIncomingCardAction(item.rawStatus);
        const canEdit = isStationary && cardAction.isEditable;
        const canDelete =
            isStationary && canDeleteIncomingByStatus(item.rawStatus);
        const updateLabel = canEdit
            ? cardAction.nextActionLabel || 'Cập nhật'
            : '';
            ...
            <TouchableOpacity
                ...
                <View style={styles.bottomRow}>
                    <Text style={styles.stepText}>
                        {item.finishedAt
                            ? `Kết thúc vào ${formatDateTime(item.finishedAt)}`
                            : '...'}
                    </Text>
                            ...
                            <>
                                <TouchableOpacity
                                    style={[
                                        ...
                                        isBusy &&
                                            styles.updateActionButtonDisabled,
                                    ]}
                                    disabled={isBusy}
                                    onPress={(event: any) => {
