...
const DropdownListItem = React.memo(
    ({item, checked, showCheckbox, onPress}: DropdownListItemProps) => {
...
const DropdownModal = React.memo(
    ({
        ...
        const selectedKeySet = useMemo(
            () => new Set(selectedKeyList),
            [selectedKeyList],
        );
        const selectedKeySignature = useMemo(
            () => selectedKeyList.join('|'),
            [selectedKeyList],
        );
        ...
        const keyExtractor = useCallback(
            (item: TDropdownListOption) => item.key,
            [],
        );

        const renderItem = useCallback(
            ({item}: {item: TDropdownListOption}) => {
                const checked =
