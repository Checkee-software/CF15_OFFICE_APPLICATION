...
export const INCOMING_ASSIGNMENT_DRAFT_STORAGE_KEY =
    '@incomingAssignmentDrafts';
...
let incomingAssignmentOptionsPromise: Promise<TAssignmentOptionsSnapshot> | null =
    null;
...
const mapDirectoryUsers = (users: any[]): TDirectoryUser[] =>
    users
        .map((user: any) => ({
            ...
            departmentId: String(
                user?.userType?.department || user?.departmentId || '',
            ),
        ...
        .sort((a: TDirectoryUser, b: TDirectoryUser) =>
...
const mapDepartmentOptions = (departments: any[]): TDepartmentOption[] =>
    departments
        ...
        .sort((a: TDepartmentOption, b: TDepartmentOption) =>
            a.name.localeCompare(b.name, 'vi'),
        );

const buildDepartmentFallbackFromUsers = (
    users: TDirectoryUser[],
): TDepartmentOption[] => {
...
const isFreshAssignmentOptionsCache = () =>
    ...
    Date.now() - incomingAssignmentOptionsCache.fetchedAt <
        ASSIGNMENT_OPTIONS_CACHE_TTL_MS;

export const loadIncomingAssignmentOptions =
    async (): Promise<TAssignmentOptionsSnapshot> => {
        ...
        incomingAssignmentOptionsPromise = (async () => {
            ...
            const users =
                usersResult.status === 'fulfilled'
                    ? mapDirectoryUsers(
                          normalizeSelectionList(usersResult.value?.data),
                      )
                    : [];
            const departmentsFromApi =
                departmentsResult.status === 'fulfilled'
                    ? mapDepartmentOptions(
                          normalizeSelectionList(departmentsResult.value?.data),
                      )
                    : [];
            const departments =
...
export const readStoredAssignmentDrafts = async (): Promise<
