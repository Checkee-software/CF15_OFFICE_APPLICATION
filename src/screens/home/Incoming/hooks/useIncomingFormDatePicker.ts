...
export const useIncomingFormDatePicker = (
  ...
  }, [clearEditorFocus, resetMenus, setFocusedField, finishedAtDisplay, finishedAtValue]);
  ...
  const applyDateValue = useCallback((iso: string) => {
    ...
    closeDatePicker();
  }, [showCreatedDatePicker, showFinishedDatePicker, closeDatePicker, setErrors]);
  ...
  const applyRelativeDate = useCallback((offsetDays: number) => {
    const d = new Date();
