...
export const useIncomingFormSubmit = (
    state: {
        ...
        setFocusedField: React.Dispatch<
            React.SetStateAction<TFocusableIncomingField | null>
        >;
        errors: any;
    ...
    const startEdit = async (
        ...
        try {
            ...
            state.setRegisteredNumberValue(
                (doc.registeredNumber || '').trimStart(),
            );
            ...
            const finishedIso = normalizeDateInput(
            ...
            const mainFilesFromMain = Array.isArray(doc.mainFiles)
                ? doc.mainFiles
                : [];
            const mainFilesFromSigned = Array.isArray(doc.signedFiles)
                ? doc.signedFiles
                : [];
            const mergedAttachedFiles = Array.isArray(doc.attachedFiles)
                ? doc.attachedFiles
                : [];
            ...
            const normalizedLeadDepartmentName =
                ...
                normalizePrimitive((doc as any).leadDepartmentName) ||
                normalizePrimitive((doc as any).receiveDepartmentName) ||
                (!isObjectId(normalizedLeadDepartmentId)
                    ? normalizedLeadDepartmentId
                    : '');
            ...
            const cachedAssignmentDraft =
                assignmentHook.assignmentDraftsRef.current[id];
            const storedAssignmentDraft =
                mode === 'LAYOUT_4'
                    ? cachedAssignmentDraft ||
                      (await getStoredAssignmentDraft(id))
                    : undefined;
            if (storedAssignmentDraft) {
                assignmentHook.assignmentDraftsRef.current[id] =
                    storedAssignmentDraft;
            }
            if (mode === 'LAYOUT_4' && storedAssignmentDraft) {
                if (!hasExplicitLeadDepartmentFromDetail) {
                    assignmentHook.setLeadDepartmentId(
                        storedAssignmentDraft.leadDepartmentId,
                    );
                    assignmentHook.setLeadAgencyValue(
                        storedAssignmentDraft.leadAgencyValue,
                    );
                    assignmentHook.setLeadAgencyPayloadValue(
                        storedAssignmentDraft.leadAgencyPayloadValue,
                    );
                }
                if (!finishedIso) {
                    datePickerHook.setFinishedAtValue(
                        storedAssignmentDraft.finishedAtValue,
                    );
                    datePickerHook.setFinishedAtDisplay(
                        storedAssignmentDraft.finishedAtDisplay,
                    );
                }
                if (receiveIds.length === 0 && !receiveFallback) {
                    assignmentHook.setReceiveToKnowIds(
                        storedAssignmentDraft.receiveToKnowIds,
                    );
                    assignmentHook.setReceiveToKnowTextFallback(
                        storedAssignmentDraft.receiveToKnowTextFallback,
                    );
                }
                if (supportIds.length === 0 && !supportFallback) {
                    assignmentHook.setSupportDepartmentIds(
                        storedAssignmentDraft.supportDepartmentIds,
                    );
                    assignmentHook.setSupportDepartmentTextFallback(
                        storedAssignmentDraft.supportDepartmentTextFallback,
                    );
                }
            }
    ...
    const appendAssignmentFields = (formData: FormData) => {
        ...
        formData.append(
            'receiveToKnow',
            JSON.stringify(assignmentHook.receiveToKnowIds),
        );
        formData.append(
            'receiveToKnowIds',
            JSON.stringify(assignmentHook.receiveToKnowIds),
        ...
        formData.append(
            'supportDepartmentId',
            JSON.stringify(assignmentHook.supportDepartmentIds),
        );
        formData.append(
    ...
    const validateBaseFields = (nextErrors: any) => {
        ...
        if (!state.organizationValue.trim())
            nextErrors.organization = 'Bắt buộc!';
        if (!state.registeredNumberValue.trim())
            nextErrors.registeredNumber = 'Bắt buộc!';
    ...
    const submitLayout1 = async (asDraft: boolean) => {
        ...
        if (!state.editingId && totalMainFiles === 0)
            nextErrors.mainFiles =
                'Vui lòng tải lên ít nhất 1 file văn bản chính!';
        ...
        try {
            ...
            if (state.editingId) {
                formData.append(
                    'mainFilesToRemove',
                    JSON.stringify(filesHook.mainFilesToRemove),
                );
                formData.append(
                    'attachedFilesToRemove',
                    JSON.stringify(filesHook.attachedFilesToRemove),
                );
            }
    ...
    const buildRegisterFormData = (contentOverride?: string) => {
        ...
        formData.append(
            ...
            assignmentHook.selectedDestinationId,
        );
    ...
    const buildAssignFormData = (
        ...
        formData.append(
            'mainFilesToRemove',
            JSON.stringify(
                options?.includeFilesToRemove === false
                    ? []
                    : filesHook.mainFilesToRemove,
            ),
        );
        formData.append(
            'attachedFilesToRemove',
            JSON.stringify(
                options?.includeFilesToRemove === false
                    ? []
                    : filesHook.attachedFilesToRemove,
            ),
        );
    ...
    const submitLayout3 = async () => {
        ...
        try {
            ...
            const assignmentDraft =
                assignmentHook.buildCurrentAssignmentDraft();
            assignmentHook.assignmentDraftsRef.current[state.editingId] =
                assignmentDraft;
