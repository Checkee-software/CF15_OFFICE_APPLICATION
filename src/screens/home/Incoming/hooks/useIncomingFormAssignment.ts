...
import { DOCUMENT_PRIORITY_LABEL, EDocumentPriority } from '@/shared-types/common/Document/document';
import {
  type TDirectoryUser,
...
export const useIncomingFormAssignment = (
  ...
  const buildOptionSearchText = useCallback((...values: Array<string | undefined>) =>
    values
  ...
  const destinationDropdownOptions = useMemo(
    ...
    [destinationLevelOptions, buildOptionSearchText],
  );
  ...
  const leadDepartmentDropdownOptions = useMemo(
    ...
    [departmentOptions, buildOptionSearchText],
  );
  ...
  const receiveDropdownOptions = useMemo(
    ...
    [directoryUsers, buildOptionSearchText],
  );
  ...
  const categoryDropdownOptions = useMemo(
    ...
    [categories, buildOptionSearchText],
  );
  ...
  const toggleDestinationMenu = useCallback((levelIndex: number) => {
  ...
  }, [clearEditorFocus, setFocusedField, ensureIncomingCategoryList, showDestinationLevel, editorFocusedRef, resetMenusRef]);
