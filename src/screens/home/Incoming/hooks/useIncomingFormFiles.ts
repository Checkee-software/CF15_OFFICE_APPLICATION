...
export const useIncomingFormFiles = (formMode: TFormMode) => {
  ...
  const handlePickAttachedFiles = useCallback(async (clearErrors: () => void) => {
    try {
      const result = await pick({
        type: [types.pdf],
        allowMultiSelection: true,
  ...
  const canRemoveExistingMainFile = useCallback((file: TExistingFile) =>
    file.source === 'signedFiles' || formMode === 'LAYOUT_1', [formMode]);
