...
export const useIncomingFormEditor = (
    showForm: boolean,
    ...
    const editorContentRequestRef = useRef<((html: string) => void) | null>(
        null,
    );
    ...
    const [activeFormat, setActiveFormat] = useState<'Paragraph' | 'H1' | 'H2'>(
        'Paragraph',
    );
    ...
    const scrollFormToEditor = useCallback((delay = 0) => {
        const scrollToEditor = () => {
            ...
            editorRef.current?.injectJavaScript(
                'window.__ensureCaretVisible && window.__ensureCaretVisible();true;',
            );
        };
