...
export const useOutgoingData = (
    ...
    const fetchDepartments = useCallback(async () => {
        ...
        try {
            ...
            const list: IDepartmentSelection[] = (res.data?.data || []).map(
                (d: any) => ({
                    _id: d._id || "",
                    name: d.name || "",
                    code: d.code || "",
    ...
    const fetchSignerUsers = useCallback(
        async (
            deptId: string,
            ...
            try {
                ...
                if (dept === ESignDepartment.MANAGEMENT) {
                    const initialSigners: TSignerUser[] = (
                        payload?.initialSigners || []
                    ).map((u: any) => ({
                        _id: u._id || "",
                        fullName: u.fullName || u.username || "",
                    }));
                    const approvers: TSignerUser[] = (
                        payload?.approvers || []
                    ).map((u: any) => ({
                        _id: u._id || "",
                        fullName: u.fullName || u.username || "",
                    ...
                    if (currentSignerId) {
                        const found = initialSigners.find(
                            u => u._id === currentSignerId,
                        );
                        if (found) {
    ...
    useEffect(() => {
        if (!signedDepartmentValue || !userDepartmentId) {
            return;
        }
