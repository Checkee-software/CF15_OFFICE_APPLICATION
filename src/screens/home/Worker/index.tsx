...
const Woker = ({navigation}) => {
    const {listWorker, listWorkerFilterByRole, getListWorker, isLoading} =
        useWorkerStore();
    ...
    const renderWorkerBySearch = (itemWorkerBySearch: any, index: number) => (
        <View style={WokerStyles.listWorkerMargin}>
            <TouchableOpacity
                ...
                <View>
                    <Text style={WokerStyles.workerOrder}>{index + 1}</Text>
                </View>
            </TouchableOpacity>
        </View>
    );

    const renderListWorker = (itemListWorker: any, index: number) => (
        <View style={WokerStyles.listWorkerMargin}>
            <TouchableOpacity
                style={WokerStyles.workerCard}
                onPress={() =>
            ...
            {searchWorker ? (
                    ...
                    <View style={WokerStyles.searchListWorker}>
                        <FlatList
                            ...
                            renderItem={({item, index}) =>
                                renderWorkerBySearch(item, index)
                            }
                            keyExtractor={(item, index) => index.toString()}
                        />
                    </View>
                ) : (
                    <View style={[WokerStyles.workerEmpty, {flex: 1}]}>
                        <Image
                ...
                <View style={WokerStyles.listWorker}>
                    <SectionList
                        ...
                        onRefresh={getListWorker}
                        refreshing={isLoading}
                        renderItem={({item, index}) =>
                            renderListWorker(item, index)
                        }
