...
export default function Main({ navigation }: any) {
    ...
    const renderNewsItem = ({ item }: any) => (
        <TouchableOpacity
            ...
            <Image
                source={item.imagePath ? { uri: getFullAvatarUrl(item.imagePath) } : images.plant1}
                style={styles.newsImage}
            />
            <View style={styles.newsContent}>
                <Text style={styles.newsType}>Loại tin tức</Text>
                <Text numberOfLines={2} style={styles.newsTitle}>{item.title}</Text>
                <Text numberOfLines={2} style={styles.newsExcerpt}>{item.content}</Text>
            ...
            <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} bounces={false} overScrollMode="never">
                {announcement && isFocused ? (
                    <View style={styles.announcementContainer}>
                        <View style={styles.announcementRow}>
                            <MaterialCommunityIcons name="lightbulb-on-outline" size={17} color="#FFFFFF" />
                            <View style={styles.announcementDivider} />

                            <GestureHandlerRootView style={styles.marqueeWrap}>
                                <Marquee frameRate={30} spacing={120} speed={1.1} withGesture={false}>
                ...
                <View style={styles.welcomeUser}>
                    <View style={styles.helloTime}>
                        <Text style={styles.helloTimeText}>{greetingValue}</Text>
                        <Text style={styles.helloUserText}>{userInfo.fullName}</Text>
                    </View>

                    <TouchableOpacity style={styles.avatarUser} onPress={() => navigation.navigate(SCREEN_INFO.PROFILE.key)}>
                        <Image
                            source={avatarSource}
                            style={styles.avatar}
                ...
                <View style={styles.heroStack}>
                    <TouchableOpacity
                        ...
                        activeOpacity={0.92}
                        onPress={() => navigation.navigate(SCREEN_INFO.PRODUCTION_PORTAL.key, { category: 'production' })}>
                        <Image source={images.cultivationArea} style={styles.heroIllustration} resizeMode="contain" />
                    ...
                    <TouchableOpacity
                        style={styles.heroCard}
                        activeOpacity={0.92}
                        onPress={() => navigation.navigate(SCREEN_INFO.OFFICE_PORTAL.key, { category: 'office' })}>
                        <View style={styles.heroContent}>
                            <Text style={styles.heroTitle} numberOfLines={1} adjustsFontSizeToFit>
                                Văn phòng <Text style={styles.heroAccent}>điện tử</Text>
                            </Text>
                        </View>
                        <Image source={images.workplaceIcon} style={styles.heroIllustration} resizeMode="contain" />
                    </TouchableOpacity>
                </View>
                ...
                <View style={styles.newsSection}>
                    <View style={styles.sectionHeader}>
                        <View style={styles.sectionTitleWrap}>
                            <MaterialCommunityIcons name="newspaper-variant-outline" size={20} color="#1B1B1B" />
                            <Text style={styles.sectionTitle}>Tin tức mới nhất</Text>
                        </View>
                        <TouchableOpacity onPress={() => navigation.navigate(SCREEN_INFO.NEWS.key)}>
                    ...
                    <FlatList
                        ...
                        scrollEnabled={false}
                        ListEmptyComponent={<View style={styles.emptyNewsBox}><Text style={styles.emptyNewsText}>Chưa có tin tức hiển thị.</Text></View>}
                    />
                </View>
            </ScrollView>
        </View>
...
const styles = StyleSheet.create({
    ...
    welcomeUser: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 },
    helloTime: { flex: 1 },
    ...
    heroCard: { minHeight: 118, backgroundColor: '#FFFFFF', borderRadius: 18, borderWidth: 1, borderColor: '#D9D9D9', paddingHorizontal: 18, paddingVertical: 18, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
    heroContent: { flex: 1, justifyContent: 'center', alignItems: 'flex-start', paddingRight: 10 },
    ...
    newsImage: { width: 96, height: 74, borderRadius: 10, backgroundColor: '#EFEFEF' },
    newsContent: { flex: 1, marginLeft: 10 },
    newsType: { fontSize: 12, color: '#A0A0A0' },
    newsTitle: { fontSize: 20, color: '#222', fontWeight: '700', marginTop: 2, marginBottom: 2 },
    newsExcerpt: { fontSize: 14, color: '#878787', lineHeight: 20 },
    newsAuthor: { marginTop: 4, fontSize: 13, color: '#4A86FF', fontWeight: '500' },
